...
**Hierarchical chunking.** Store documents at multiple granularities — full document, section, paragraph — and retrieve at the right level based on the query type. Broad conceptual questions retrieve section-level; specific factual questions retrieve paragraph-level.

**Agentic RAG.** For complex questions that require multiple lookups, turn the retrieval step into an agentic loop: retrieve, evaluate whether the retrieved content is sufficient, retrieve again with a refined query if not, synthesize when confident. This is slower but handles multi-hop questions that single-shot retrieval can't.
