...
| [premortem](workflow/premortem/) | Before committing to any plan with real stakes — client money, public launch, >1 week investment. Run after spec is confirmed, before architecture. MANDATORY for SignalWorks engagements. | Assumes plan already failed 6 months from now; spawns one parallel agent per failure reason; synthesizes most-likely failure, most-dangerous failure, hidden assumption, revised plan, and pre-launch checklist. Outputs HTML report + transcript. |
...
| [sweep](production/sweep/) | "Going public", "client delivery", before any deploy declaration. Stronger gate than pre-ship for high-stakes contexts. | **Multi-agent deployment gate** — 9-12 specialized sub-agents in 2 waves + verification round across security/code/infra/data/UX/docs territories with heavy KB context (OWASP LLM Top 10, pre-ship checklist, A1-A20 anti-patterns, ai-security §3-§4). Hard pass-gate (GO/HOLD/NO-GO). Conditional agents fire when warranted: ai-eval-integrity (Type A), ai-rag-security (RAG), compliance-specific (HIPAA/GDPR/ABA/SOC2), cost-controls (Type A), performance-load (public+traffic). Catches all P1 issues in one sweep. Does NOT replace manual eyes-on. |
...
| [http-client-hygiene](production/http-client-hygiene/) | Writing or reviewing any HTTP client code that calls a third-party API with a secret (especially `?api_key=` query-param auth) | 4-point checklist preventing API key leaks through `requests` exception stringification, `__cause__` chains, and default traceback printing. Includes scrubbed client skeleton + regression test patterns. |
