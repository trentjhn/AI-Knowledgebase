...
- **Catalog entry mandate:** When the skill-activation hook surfaces "RELEVANT KB CATALOG ENTRIES," read the SKILL.md or agent definition before generating any response. This is not optional — treat it as a mandatory context load, the same as invoking an installed skill.
- **Memory topic file mandate:** When a MEMORY.md pointer is directly relevant to the current task, read the topic file before responding. The one-line MEMORY.md summary is an index entry, not the full context.
